...
for (const control of ["OpenReplayInspector", "ShowReplayInspector", "DETERMINISM AUDIT", "PLAY TO END", "RESET INSPECTION", "TIMELINE", "PREVIOUS", "NEXT", "HSlider", "_Input", "KEYS ·", "ReplayInspectorKeyBindings", "REPLAY INSPECTOR KEYS"]) {
  if (!uiSource.includes(control)) throw new Error(`Missing command-table replay inspector control: ${control}`);
}
for (const contract of ["ReplayInspectorShortcut", "ReplayInspectorKeyBindings", "TryAssign", "TryResolve", "RestoreDefaults", "Normalize", "Previous", "Next", "Start", "End", "PlayToEnd"]) {
