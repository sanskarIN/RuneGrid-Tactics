...
const required = ["project.godot", "RuneGrid.Tactics.csproj", "Scenes/Main.tscn", "Scripts/Godot/GameRoot.cs", "Scripts/Godot/BoardView.cs", "Scripts/Core/GameSession.cs", "Scripts/Core/ReplayPlayer.cs", "Scripts/Core/ReplayFingerprint.cs", "Scripts/Core/ReplayStateDiff.cs", "Scripts/Core/ReplayInspector.cs", "Scripts/Core/ReplayInspectorShortcuts.cs", "Scripts/Core/ReplayInspectorOnboarding.cs", "Scripts/Core/ReplayDiffFilter.cs", "Scripts/Core/ReplayMismatchExport.cs", "export_presets.cfg", "Tests/RuneGrid.Tactics.Pathfinding.Tests.csproj", "Tests/TacticalGridPathfindingTests.cs"];
...
const exportSource = await readFile(join(root, "Scripts", "Core", "ReplayMismatchExport.cs"), "utf8");
...
const serviceSource = await readFile(join(root, "Scripts", "Godot", "GameServices.cs"), "utf8");
...
for (const contract of ["ReplayDiffCategory", "ReplayDiffEntry", "ReplayDiffFilterResult", "ReplayDiffFilter", "ReplayDiffFilterShortcut", "ReplayDiffFilterNavigator", "ReplayDiffFilterShortcutReference", "Filter", "Classify", "TryParseShortcut", "TryParseDirectShortcut", "Previous", "Next", "AffectedTiles", "AffectedUnitIds"]) {
  if (!diffFilterSource.includes(contract)) throw new Error(`Missing replay diff filter contract: ${contract}`);
}
for (const contract of ["ReplayMismatchExport", "ReplayMismatchExportLine", "ReplayMismatchExportBuilder", "ReplayMismatchAutoExportState", "ReplayMismatchAutoExport", "ToJson", "ToCsv", "BuildFileName", "HasExported", "MarkExported", "BuildUserPath", "AffectedTiles", "AffectedUnits", "Differences"]) {
  if (!exportSource.includes(contract)) throw new Error(`Missing replay mismatch export contract: ${contract}`);
}
for (const contract of ["AutoExportReplayMismatchReports", "ReplayMismatchAutoExportState"]) {
  if (!serviceSource.includes(contract)) throw new Error(`Missing replay auto-export persistence contract: ${contract}`);
}
...
console.log(`Godot project structure, ${jsonFiles.length} JSON content files, advanced routing, replay determinism, diagnostics, inspection, timeline scrubbing, keyboard navigation, shortcut reference overlay, first-time onboarding, mismatch warnings, filtered diff markers, keyboard filter cycling, direct numeric filter selection, focused mismatch export, and automatic mismatch export validated.`);
