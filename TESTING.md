...
| Automatic mismatch export | An opt-in local preference writes one full-audit JSON record per unique action-and-fingerprint mismatch signature. Exported signatures persist in a bounded local set, making repeated renders idempotent while allowing changed mismatches to export. |
...
The optional **Automatically export replay mismatch reports** setting writes a deterministic full-audit JSON report to `user://` when a new replay mismatch appears. It records the successful mismatch signature only after the file is written, so a transient write failure remains eligible for a later retry. The replay inspector then displays a compact local-path notice; navigating back to a matching state clears it. The bounded signature set prevents duplicate exports when an inspector re-renders the same divergence.
